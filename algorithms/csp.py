...
def graph_coloring_demo(colors: tuple[str, ...] = ("Red", "Green", "Blue")) -> SearchResult:
    """Solve a small map-coloring CSP that is separate from the 15-puzzle."""
    variables = list(AUSTRALIA_GRAPH)
    assignment: dict[str, str] = {}
    trace: list[TraceStep] = []
    steps = 0

    def is_valid(region: str, color: str) -> bool:
        return all(assignment.get(neighbor) != color for neighbor in AUSTRALIA_GRAPH[region])

    def select_unassigned_variable() -> str:
        unassigned = [region for region in variables if region not in assignment]
        return max(unassigned, key=lambda region: len(AUSTRALIA_GRAPH[region]))
            ...
            if is_valid(region, color):
                ...
                if len(trace) < 80:
                    trace.append(TraceStep(
                        step=steps,
                        state=GOAL_STATE,
                        reason=f"Assign {region}={color}; partial={dict(assignment)}",
                    ))
                if backtrack():
                    return True
    ...
    message = (
        "Graph Coloring CSP Demo: Australia Map\n\n"
        "This is the correct home for the coloring algorithm. It is a CSP/game-style "
        "teaching example, not a natural 15-puzzle solver.\n\n"
        "Variables: regions {WA, NT, SA, Q, NSW, V, T}\n"
        f"Domains: colors {{{', '.join(colors)}}}\n"
    ...
    return SearchResult(
        success=solved,
        algorithm="Graph Coloring Demo",
        group="CSP",
        nodes_expanded=steps,
        nodes_generated=steps,
        message=message,
        trace=trace,
