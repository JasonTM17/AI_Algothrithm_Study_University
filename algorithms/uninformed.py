...
def bfs(
    ...
    if rejected_duplicates > 0:
        msg += f" ({rejected_duplicates} duplicate{'s' if rejected_duplicates != 1 else ''} rejected across {nodes_generated} generated nodes)"
    return _make_result(False, "BFS", None, start, goal, nodes_expanded, nodes_generated, max_frontier, len(reached), t0, trace, msg, True, True)


def _ancestor_states(node: Node) -> set[tuple[int, ...]]:
    states = set()
...
def dfs(
    ...
    best_depth: dict[tuple[int, ...], int] = {start: 0}
    nodes_expanded = 0
    nodes_generated = 1
    max_frontier = 1
    trace: list[TraceStep] = []
    ...
    while frontier:
        if time.perf_counter() - t0 > timeout:
            return _make_result(False, "DFS", None, start, goal, nodes_expanded, nodes_generated, max_frontier, len(best_depth), t0, trace, "Timeout", False, False)
        if len(best_depth) > max_nodes:
            return _make_result(False, "DFS", None, start, goal, nodes_expanded, nodes_generated, max_frontier, len(best_depth), t0, trace, f"Node limit exceeded ({max_nodes})", False, False)

        node = frontier.pop()
        if node.depth > best_depth.get(node.state, node.depth):
            continue
        nodes_expanded += 1
        ...
        ps = PuzzleState(node.state)
        ancestor_states = _ancestor_states(node)
        for ns, action, cost in reversed(ps.get_neighbors(action_order)):
            if time.perf_counter() - t0 > timeout:
                return _make_result(False, "DFS", None, start, goal, nodes_expanded, nodes_generated, max_frontier, len(best_depth), t0, trace, "Timeout", False, False)

            if ns in ancestor_states:
                if len(trace) < 200:
                    trace.append(TraceStep(
                        step=nodes_expanded, state=ns, action=action,
                        ...
                        reached_states=list(best_depth.keys())[:12],
                        reason=f"Rejected ancestor cycle, action={action}",
                    ))
                continue
            ...
            if ns == goal:
                return _make_result(True, "DFS", child, start, goal, nodes_expanded, nodes_generated, max_frontier, len(best_depth), t0, trace, "Solution found", False, False)

            prev_depth = best_depth.get(ns)
            if prev_depth is not None and child.depth >= prev_depth:
                continue

            best_depth[ns] = child.depth
            frontier.append(child)
            max_frontier = max(max_frontier, len(frontier))
    ...
    return _make_result(False, "DFS", None, start, goal, nodes_expanded, nodes_generated, max_frontier, len(best_depth), t0, trace, "No solution found within depth limit", False, False)
...
def _dls(start, goal, depth_limit, action_order, t0_global, timeout, max_nodes,
    ...
    if timed_out[0]:
        return SearchResult(
            success=False, message="Timeout", termination_reason="timeout",
            nodes_expanded=nodes_expanded[0], nodes_generated=nodes_generated[0],
            max_frontier_size=max_frontier[0], trace=global_trace,
        )
    if resource_limited[0]:
        return SearchResult(
            success=False, message=f"Node limit exceeded ({max_nodes})", termination_reason="resource_limit",
            nodes_expanded=nodes_expanded[0], nodes_generated=nodes_generated[0],
            max_frontier_size=max_frontier[0], trace=global_trace,
        )
    if found_cutoff[0]:
        return SearchResult(
            success=False, message=f"cutoff at depth {depth_limit}", termination_reason="cutoff",
            nodes_expanded=nodes_expanded[0], nodes_generated=nodes_generated[0],
            max_frontier_size=max_frontier[0], trace=global_trace,
        )
    return SearchResult(
        success=False, message="No solution at this depth", termination_reason="exhausted",
        nodes_expanded=nodes_expanded[0], nodes_generated=nodes_generated[0],
        max_frontier_size=max_frontier[0], trace=global_trace,
    )
