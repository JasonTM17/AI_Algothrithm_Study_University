...
| CSP | Definition, Propagation, Path Consistency, Global Constraints, Backtracking, Min-Conflicts, Constraint Graphs | Dung vai tro minh hoa, rang buoc, bounded planning, khong claim solver tu nhien. |
...
| Randomness | Thuat toan stochastic phai ghi seed; fixed seed phai tai lap duoc. |
...
| ALG-11 | Stochastic seeds | Stochastic HC/SA benchmark | Fresh seeds khac nhau; seed duoc hien thi va luu trong result. |
| ALG-12 | Tournament optimal score | A* tren `ONE_MOVE` | Agent nhan 100 diem, optimal cost duoc reference dung chung. |
| ALG-13 | Tournament reduced score | Legal path dai hon optimal | Agent nhan diem duong thap hon 100 theo excess cost. |
...
| Run Algorithm | Tham so solver co label ro, loading feedback khi chay, result metric khong che trace, Frontier co legend ky hieu. |
...
| Theory/PEAS | Khong co thuat toan trong taxonomy thieu theory data; ranh gioi CSP/game/no-observation/tournament duoc nhac ro. |
| Advanced | Mode AI-vs-AI Tournament chay duoc; hien reference optimal cost, ly do cham diem, tong diem, winner/draw. |
...
- Advanced: chay AI-vs-AI Tournament voi A* vs Greedy tren board mot buoc, winner/draw hien dung.
