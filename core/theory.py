...
THEORY["AI-vs-AI Tournament"] = {
    ...
    "goal": "Chấm điểm hai AI giải cùng một bài 15-puzzle bằng chuẩn A* tối ưu.",
    "goal_en": "Score two AI agents on the same 15-puzzle instance using an A* optimal reference.",
    "idea": "Môi trường puzzle vẫn single-agent. Tournament chạy hai solver riêng, kiểm path hợp lệ, so với optimal cost, rồi cộng/trừ điểm.",
  ...
  winner = compare(total, solved_rounds, excess_cost, runtime, nodes)""",
    "application": "Dùng để so sánh AI tìm đúng, tìm đường kém tối ưu, dừng giữa chừng, timeout, hoặc ghi path sai.",
    "application_en": "Use it to compare agents that find optimal paths, suboptimal paths, partial paths, timeouts, or invalid paths.",
    "suitable": "Phù hợp là lớp đánh giá solver. Không phải game đối kháng tự nhiên.",
    "suitable_en": "Suitable as a solver-evaluation layer. It is not a natural adversarial game.",
    "pros": ["So sánh hai AI trên cùng board", "Có chuẩn A* tối ưu", "Phát hiện path sai và timeout rõ ràng"],
    "pros_en": ["Compares two AI agents on the same board", "Uses an A* optimal reference", "Makes invalid paths and timeouts explicit"],
    "cons": ["Phụ thuộc A* reference chạy được", "Không thay thế benchmark solver chuẩn", "Không phải PEAS đối kháng"],
