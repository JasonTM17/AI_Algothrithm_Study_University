...
class SearchResult:
    ...
    def _build_search_tree_evidence(self) -> None:
        ...
        def add_node(state: tuple[int, ...], depth: int, g: float, h: float, f: float) -> SearchTreeNode:
        ...
        if solution_states:
            for index, state in enumerate(self.path):
                add_node(state, index, index, 0, index)
                if index:
        ...
        for event in self.trace:
            ...
            if _move_blank(parent_state, event.action) != child_state:
                continue
            parent = add_node(parent_state, max(int(event.g) - 1, 0), max(event.g - 1, 0), 0, max(event.g - 1, 0))
...
def search_tree_to_dot(result: SearchResult, max_nodes: int = 40) -> str:
    ...
    for node in visible.values():
        ...
        label = f"{node.node_id} | d={node.depth} g={node.g:g} h={node.h:g} f={node.f:g}\\n{grid}"
